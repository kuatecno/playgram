...
export default async function DashboardPage() {
  ...
  if (adminId) {
    const [contactsCount, qrStats, bookingsCount, chatStats] = await Promise.all([
      db.user.count(),
      qrCodeService.getQRCodeStats(adminId),
      db.booking.count({
        where: {
          tool: { adminId },
          bookingDate: {
      ...
      aiChatService.getChatStats(adminId),
    ])

    totalContacts = contactsCount
    totalQRCodes = qrStats.total
    upcomingBookings = bookingsCount
    totalConversations = chatStats.totalInteractions
  }
