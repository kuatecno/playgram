...
export default function DynamicGalleryDetailPage() {
  ...
  const loadGallery = async () => {
    try {
      setIsLoading(true)
      const res = await fetch(`/api/v1/dynamic-gallery/galleries/${galleryId}`)
      const data = await res.json()

      if (data.success) {
        setGallery(data.data)
        // Also load the old-style summary for backward compatibility with existing code
        const summaryData = await dynamicGalleryApi.getSummary()
        setSummary(summaryData)
      } else {
        toast({
          title: 'Error',
          description: data.error || 'Failed to load gallery',
          variant: 'destructive',
        })
      }
    } catch (error) {
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <Button
              ...
              size="sm"
              className="gap-2"
              onClick={() => router.push('/engagement/dynamic-gallery')}
            >
              <ArrowLeft className="h-4 w-4" />
              Back to Galleries
            </Button>
          </div>
          <h1 className="text-3xl font-bold">{gallery?.name || 'Dynamic Gallery'}</h1>
