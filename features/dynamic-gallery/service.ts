  ...
  async syncToManychat(adminId: string, options: {
    ...
    for (const contactId of targetContactIds) {
      for (let index = 0; index < cards.length; index++) {
        ...
        await manychatService.setCustomField(adminId, contactId, `${baseField}_image_url`, card.imageUrl)
        await manychatService.setCustomField(adminId, contactId, `${baseField}_title`, card.title)
        await manychatService.setCustomField(adminId, contactId, `${baseField}_subtitle`, card.subtitle)
        ...
        for (let buttonIndex = 0; buttonIndex < card.buttons.length; buttonIndex++) {
          ...
          await manychatService.setCustomField(adminId, contactId, `${buttonField}_title`, button.title)
          if (button.url) {
            await manychatService.setCustomField(adminId, contactId, `${buttonField}_url`, button.url)
          }
          if (button.value) {
            await manychatService.setCustomField(adminId, contactId, `${buttonField}_value`, button.value)
          }
          await manychatService.setCustomField(adminId, contactId, `${buttonField}_type`, button.type)
        }
      }

      await manychatService.setCustomField(adminId, contactId, 'playgram_gallery_active_count', cards.length)
      await manychatService.setCustomField(adminId, contactId, 'playgram_gallery_last_updated_iso', new Date().toISOString())
      updatedCount += 1
    }
